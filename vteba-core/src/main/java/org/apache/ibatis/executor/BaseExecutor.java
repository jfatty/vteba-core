...
public abstract class BaseExecutor implements Executor {
	...
	protected BaseExecutor(Configuration configuration, Transaction transaction) {
		...
		this.localOutputParameterCache = new PerpetualCache(
	...
	public void close(boolean forceRollback) {
		try {
			...
			log.warn("Unexpected exception on closing transaction.  Cause: "
					+ e);
		} finally {
			transaction = null;
			deferredLoads = null;
	...
	public void deferLoad(MappedStatement ms, MetaObject resultObject,
		...
		if (deferredLoad.canLoad()) {
			...
			deferredLoads.add(new DeferredLoad(resultObject, property, key,
	...
	private void handleLocallyCachedOutputParameters(MappedStatement ms,
			CacheKey key, Object parameter, BoundSql boundSql) {
		if (ms.getStatementType() == StatementType.CALLABLE) {
			final Object cachedParameter = localOutputParameterCache
					.getObject(key);
			if (cachedParameter != null && parameter != null) {
				final MetaObject metaCachedParameter = configuration
						.newMetaObject(cachedParameter);
				final MetaObject metaParameter = configuration
						.newMetaObject(parameter);
				for (ParameterMapping parameterMapping : boundSql
						.getParameterMappings()) {
					if (parameterMapping.getMode() != ParameterMode.IN) {
						final String parameterName = parameterMapping
								.getProperty();
	...
	private static class DeferredLoad {
		...
		public DeferredLoad(MetaObject resultObject, String property,
				CacheKey key, PerpetualCache localCache,
		...
		public void load() {
			...
			Object value = resultExtractor.extractObjectFromList(list,
					targetType);
